...
class Maker
{
	...
	public function setHtmlIndex($config)
	{
		...
		$html_th .= "\n\t\t\t\t\t\t<th>ACTION</th>";
		$html_td .= "\n\t\t\t\t\t\t<td><?= Template::link('&#9998;', ROOT.'".$config['table']."'.DS.'edit'.DS.\$row->id, ['class' => 'link mx-1', 'title' => 'Edit']); ?>\n\t\t\t\t\t\t\t<?= Template::link('&#10060;', null, [\n\t\t\t\t\t\t\t\t'title' 	  => 'Delete',\n\t\t\t\t\t\t\t\t'class' 	  => 'link mx-1 delete',\n\t\t\t\t\t\t\t\t'data-title'  => 'Delete '.\$row->id.'?',\n\t\t\t\t\t\t\t\t'data-body'   => 'Do you want to delete the '.\$row->id.'?',\n\t\t\t\t\t\t\t\t'data-footer' => json_encode(array(\n\t\t\t\t\t\t\t\t\t'Delete' => ['class'=>'btn btn-danger','href'=> ROOT.'".$config['table']."'.DS.'delete'.DS.\$row->id],\n\t\t\t\t\t\t\t\t\t'Cancel' => ['class'=>'btn btn-light', 'data-dismiss'=>'modal']\n\t\t\t\t\t\t\t\t))\n\t\t\t\t\t\t\t]); ?>\n\t\t\t\t\t\t</td>";
		$html 	 .= "<div class='row'>\n\t<div class='col-12'>\n\t\t<div class='table-responsive'>\n\t\t\t<table class='table table-sm'>\n\t\t\t\t<thead class='thead-light'>\n\t\t\t\t\t<tr>".$html_th."\n\t\t\t\t\t</tr>\n\t\t\t\t</thead>\n\t\t\t\t<tbody>\n\t\t\t\t<?php foreach (\$".$config['table']." as \$row) { ?>\n\t\t\t\t\t<tr>".$html_td."\n\t\t\t\t\t</tr>\n\t\t\t\t<?php } ?>\n\t\t\t\t</tbody>\n\t\t\t</table>\n\t\t</div>\n\t</div>\n</div>\n";
	...
	public function setHtmlCreate($config)
	{
		...
		foreach ($config['columns'] as $field) {
			...
			if($field->field != 'id'){
				switch ($type) {
					case 'int':
						$default = empty($field->default) ? "\$".$config['table']."->".$field->field : "\$".$config['table']."->".$field->field." ?: '".$field->default."'" ;
					...
					case 'enum':
						...
						$html   .= "\n\t\t<div class='col-3'>\n\t\t\t<div class='form-group'>\n\t\t\t\t<?= Template::select('".$field->field."',\n\t\t\t\t\t['class' => 'form-control', 'required' => ".$required."],\n\t\t\t\t\t['label' => true, 'selected' => \$".$config['table']."->".$field->field.", 'options' => ".$options."]\n\t\t\t\t); ?>\n\t\t\t</div>\n\t\t</div>";
						break;
					case 'text':
						$html .= "\n\t\t<div class='col-12'>\n\t\t\t<div class='form-group'>\n\t\t\t\t<?= Template::textarea('".$field->field."', ['rows' => 5, 'class' => 'form-control', 'required' => ".$required."], ['label' => true, 'value' => \$".$config['table']."->".$field->field."]); ?>\n\t\t\t</div>\n\t\t</div>";
						break;
					case 'date':
						$default = empty($field->default) ? "(\$".$config['table']."->".$field->field.") ? \$".$config['table']."->".$field->field."->format('Y-m-d') : ''" : "(\$".$config['table']."->".$field->field.") ? \$".$config['table']."->".$field->field."->format('Y-m-d') : date('Y-m-d')" ;
					...
					case 'datetime':
						...
						$default 	   = empty($field->default) ? "(\$".$config['table']."->date) ? \$".$config['table']."->".$field->field."->format('Y-m-d\TH:i') : ''" : "(\$".$config['table']."->date) ? \$".$config['table']."->".$field->field."->format('Y-m-d\TH:i') : date('Y-m-d\TH:i')" ;
						$html 		  .= "\n\t\t<div class='col-3'>\n\t\t\t<div class='form-group'>\n\t\t\t\t<?= Template::input('".$field->field."', ['type' => 'datetime-local', 'class' => 'form-control', 'value' => ".$default.", 'required' => ".$required."]); ?>\n\t\t\t</div>\n\t\t</div>";
						break;
				}
			}
		}
		$html .= "\n\t</div>\n\t<hr>\n\t<div class='row'>\n\t\t<div class='col-12'>\n\t\t\t<div class='form-group'>\n\t\t\t\t<?= Template::button('Save', ['class' => 'btn btn-success']); ?>\n\t\t\t\t<?= Template::link('Back', ROOT.'".$config['table']."', ['class' => 'btn btn-warning']); ?>\n\t\t\t</div>\n\t\t</div>\n\t</div>\n</form>\n";
		$html .= "<?php require_once VIEWS.'footer.php'; ?>";
